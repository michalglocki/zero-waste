...
`npm test` via jest-expo; local Supabase fixtures (2 households / 2 users + anon + non-member); automated A≠B / membership / anon proofs; #2b allow encoded as baseline/alarm; Protected truth-table; test-plan §6.2 filled and §2 lightly corrected.
...
| Test DB | Local `supabase start` + migrations | Cheapest repeatable harness before CI Phase 4 | Plan |
...
| 2. Fixture harness | Local DB seed + JWT clients | False green if DB down; service role leaks into asserts |
...
**Prerequisites:** Docker/Supabase CLI for local DB; Expo SDK 56 project as today  
**Estimated effort:** ~2–3 sessions across 5 phases

## Open Risks & Assumptions

- Local Supabase CLI/Docker availability on contributor machines (CI deferred).
